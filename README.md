...
- 웹에서 서버 기능을 수행하는 프로그램으로 HTML문서나 JPG, PNG 같은 이미지를 HTTP프로토콜을 통해 웹 브라우저에 제공하는 서버
- 서버 내부의 이미지 만들어져 있는 정적인 요소들을 화면에 제공하는 역할
...
- Tomcat : Apache Software Foundation에서 Servlet
  과 JSP를 통한 동적인 웹 문서를 처리하기 위해 만든 웹 애플리케이션 서버

# 서블릿(Servlet)

- 웹 서비스를 위한 자바 클래스를 말하며 자바를 사용해서 웹을 만들기 위한 기술
- 웹 브라우저부터 요청을 받아 처리하고 결과를 다시 웹 브라우저로 전송하는 역할

## 동작 구조

1. 클라이언트가 웹 브라우저를 통해 요청을 보냄
2. 해당 요청이 서블릿에 대한 요청이면 서블리 컨테이너에게 처리를 넘긴다.
3. 서블릿에서 요청에 대한 실행이 완료된 후에는 서블릿 컨테이너가 실행 결과를 웹서버에 전달한다.
4. 웹서버는 서비스를 요청한 클라이언트에 응답한다.
5. 컨테이너의 서블릿에 대한 설정은 배포 서술자()에 작성한다.
...
- setCharacterEncoding(String) : 전송받은 request객체의 값들의 CharacterSet을 설정해 주는 메소드
...
3. 최종적으로 서블릿이 서비스 되지 않았을 때 destory() 메소드를 호출한다.
...
- Servlet : Java 코드에 HTML 코드 삽입,
- JSP : HTML 코드에 Java 코드 삽입

## JSP 실행 과정

- 웹 컨테이너는 클라의언트로부터의 요청이 JSP에 대한 요청일 경우 JSP파일을 자바 코드(서블릿)로 변환한다.
...
- JSP에서 사용하는 커스텀 태그
- JSP 페이지에서 사용하는 코드들을 사용하기 쉽게 태그로 만들어 표준으로 제공한다.

### JSTL 라이브러리 등록

- 톰캣 (http://tomcat.apache.org)에서 JSTL 라이브러리를 다운로드한다.
- 웹 프로젝트의 lib 폴더에 다운로드한 JSTL 라이브러리 붙여 넣는다.
- JSTL 라이브러리는 JSP 페이지에서 tanhlib 지시자로 선언해야 사용이 가능한다.

```
<%@ tagblib uir="http://java.sun.com/jsp/jstl/core" prefix="c" %>
...
- 데이터 형식은 xml뿐만 아니라 text, HTML, **JSON**, CSV 등 다양한 형식 사용 가능
...
- success(data) : ajax 통신 성공 시 호출되는 함수를 지정, 매개변수로 응답 데이터를(data) 받음
