...
public class ProductServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//컨텍스트 값 가져오기 -> 위에 값 지정해주기 private ServletContext context;
		...
		//2. 세션에 바인딩된 값이 있다면 그 값을 찾아온다. ->  LoginServlet에서 세션에 바인딩해준값 가져오기
		MemberVO vo = (MemberVO) session.getAttribute("vo"); //add cast 형변환 해주기
		if(vo!=null) { //로그인 된 상태
			out.println("VO ::"  + vo);
			out.println("<hr>");
