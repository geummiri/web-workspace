...
public class CartServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("vo")!=null) { //로그인이 정상적으로 된다면
			session.invalidate();
			out.println("<script>");
			out.println("alert('로그아웃!')");
//			out.println("location.href='index.html'"); //로그아웃 창 누른 후 처음 해당 페이지로 이동됨
			out.println("location.href='ResultServlet'"); //로그아웃 한 이후 페이지 설정
